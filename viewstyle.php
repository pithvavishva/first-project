...
?>
<?php
// Subcategory ID from URL
$id = isset($_GET['id']) ? intval($_GET['id']) : 0;

// Check if subcategory_id is provided, and update query accordingly
if ($id > 0) {
    $productqry = "SELECT * FROM products2 WHERE id = $id ORDER BY proid DESC";
} else {
    $productqry = "SELECT * FROM products2 ORDER BY id DESC";
}

$productresult = mysqli_query($conn, $productqry) or die("Product2 query failed: " . mysqli_error($conn));
?>
...
<div class="col-lg-3 col-md-4 col-sm-6 mb-4">
  ...
  <div class="product-image-wrapper mb-2">
    <img src="images/products2/<?php echo htmlspecialchars($productrow['image']); ?>"
         alt="<?php echo htmlspecialchars($productrow['proname']); ?>"
         class="img-fluid">
  </div>

  <form action="cart.php" method="post">
    <!-- ✅ Send all necessary product data -->
    <input type="hidden" name="product_name" value="<?php echo htmlspecialchars($productrow['proname']); ?>">
    <input type="hidden" name="product_price" value="<?php echo htmlspecialchars($productrow['proprice']); ?>">
    <input type="hidden" name="product_image" value="products2/<?php echo htmlspecialchars($productrow['image']); ?>">
   ...
   <a href="add_whishlist.php?product_id=<?= $productrow['id'] ?>" title="Add to Wishlist">
    ...
    <h5 class="product-title"><?php echo htmlspecialchars($productrow['proname']); ?></h5>
    <h5 class="product-title"><?php echo htmlspecialchars($productrow['prodescription']); ?></h5>
    <p class="product-price">₹<?php echo number_format($productrow['proprice'], 2); ?></p>
