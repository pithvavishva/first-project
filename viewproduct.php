...
if (isset($_POST['add_to_cart'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM cart WHERE productname = ? AND user_id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO cart(user_id, productname,productdescription, productprice, image, quantity) VALUES(?, ?,?, ?, ?, ?)");
        $stmt->bind_param("issdsi", $user_id, $product_name, $product_description, $product_price, $product_image, $product_quantity);
...
// Check if catid and subcatid are valid
if (isset($_GET['catid']) && isset($_GET['subcatid']) && is_numeric($_GET['catid']) && is_numeric($_GET['subcatid'])) {
    $catid = (int)$_GET['catid'];
    $subcatid = (int)$_GET['subcatid'];

    // Fetch products based on category and subcategory
    $productqry = "SELECT * FROM products WHERE catid = '$catid' AND subcatid = '$subcatid' ORDER BY id DESC";
    $productresult = mysqli_query($conn, $productqry) or die("Product query failed: " . mysqli_error($conn));
...
<div class="col-lg-3 col-md-4 col-sm-6 mb-4">
  <div class="product-card text-center shadow-sm p-3 rounded">
    <div class="hover-icons">
      <a href="add-to-cart.php?id=<?php echo $productrow['id']; ?>" title="Add to Cart">
        <i class="fas fa-shopping-cart"></i>
      </a>
      <a href="add-to-wishlist.php?id=<?php echo $productrow['id']; ?>" title="Add to Wishlist">
    ...
    <a href="product-details.php?id=<?php echo $productrow['id']; ?>">
      <div class="product-image-wrapper mb-2">
        <img src="images/products/<?php echo htmlspecialchars($productrow['image']); ?>"
             alt="<?php echo htmlspecialchars($productrow['productname']); ?>"
             class="img-fluid">
      </div>
       <form action="cart.php" method="post">
   <input type="hidden" name="product_name" value="<?= htmlspecialchars($productrow['productname']); ?>">
   <input type="hidden" name="product_price" value="<?= htmlspecialchars($productrow['productprice']); ?>">
    <input type="hidden" name="product_image" value="<?= htmlspecialchars($productrow['image']); ?>"> <!-- ✅ Add this -->

  <!-- ✅ Wishlist icon fixed -->
  <a href="add_whishlist.php?product_id=<?= $productrow['id'] ?>" title="Add to Wishlist">
   ...
   <h5 class="product-title"><?= htmlspecialchars($productrow['productname']); ?></h5>
   <h5 class="product-title"><?= htmlspecialchars($productrow['productdescription']); ?></h5>
   <p class="product-price">₹<?= number_format($productrow['productprice'], 2); ?></p>
   ...
   <button type="submit" name="add_to_cart" class="btn btn-primary w-100">Buy Now</button>
    ...
    <?php
        ...
        echo "<p class='text-center'>No products found in this subcategory.</p>";
    }
    ?>
  </div>
</div>
